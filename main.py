...
Backward Elimination: It starts with a subset with all features. Each iteration, the algorithm removes one feature at a time 
    ...
    while len(feature_subset) < data.shape[1] - 1:
        ...
        for feature in range(1, data.shape[1]):
            if feature not in curr_set:
                ...
                test_set.append(feature) # append the feature to the test set
                temp_acc = calc_accuracy(data, test_set) # calculate the accuracy of the test set
        ...
        # this if statement keeps track of the best subset/accuracy across all levels
...
def backward_elimination(data):
    ...
    # overall, backward elimination is similar to forward selection, but in reverse.
    ...
    while len(feature_subset) > 0:
        ...
        for feature in curr_set:
            test_set = curr_set.copy()
            test_set.remove(feature) # instead of append, we remove the feature
...
"""
calc_accuracy function calculates the accuracy of the dataset that only contains the features in the given subset
...
Note: I have the optimized version of the distance calculation using np.sum() and np.argmin() vectorized operations, which is more efficient 
      than the original version that uses nested loops. The original version is commented out at the bottom for reference.
"""
def calc_accuracy(data, feature_subset):
    ...
    # Here, 'i' is the level of the algorithm. Each level will find the best feature to remove from the current feature subset. 
    ...
    for i in range(n):
        ...
        # I consult that the vecotorized method is advised from AI tool to optimize the runtime (Large dataset used to take 17 hours to run).
        # But all the codes are written by me, and the original version is commented out below for reference.
        diff = features - features[i] 
        
        # I consult that the improved distance calculation, which uses np.sum() instead of sqrt(), is learned by stackOverflow,
        # which is calcualted by summing the squared differences, to optimize the runtime, and is sufficient because the ordering 
        # stays the same for nearest neighbor.
        ...
        #     if k != instance:
                  # in the original version, the distance is calculated using sqrt(), which is less efficient than just summing the squared differences.
